...
for i=1:length(tracklist)
  ...
  for msgNum=1:length(midi.track(tracknum).messages)
    ...
    %gives real time
    % seconds = seconds + ...
                %deltatime*1e-6*current_tempo/midi.ticks_per_quarter_note; 
    
    % this gives in 1/16ths
    seconds = seconds + deltatime*16/(4*midi.ticks_per_quarter_note);    
...
%    name = ['UNKNOWN META EVENT: ' num2str(type)]; dataStr = ...
